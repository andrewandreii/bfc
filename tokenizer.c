...
token_t *tokenize(FILE *stream) {
    ...
    int id_len [[maybe_unused]] = ID_LEN;
    char c;
    ...
    while (!feof(stream)) {
        ...
        t_list[t_top].source_file_pos = (source_file_pos_t){ .line_no = line_no, .col_no = char_no + 1 };
        ...
        if (isdigit(c) /* || ((c == '+' || c == '-') && isdigit(next_c)) */) {
            ...
            for (i = 0; keywords[i]; ++ i) {
                if (id_buffer[0] == keywords[i][0] && !strcmp(id_buffer, keywords[i])) {
                    ...
                    t_list[t_top].val.str = malloc(strlen(id_buffer) + 1);
